			...
			<div class="col-md-7">
				...
				<div class="card p-4">
					...
					<form action="updateOffer" method="post">
							...
							<label class="form-label">Car Listing</label> <select
								...
								<c:forEach items="${allCarList}" var="l">
									<option value="${l.listingId}"
										...
										${l.brandName} - ${l.modelName}</option>
								</c:forEach>
							</select>
						</div>
						...
						<!-- Offered Price -->
						<div class="mb-3">
							<label class="form-label">Offered Price</label> <input
						...
						<div class="mb-3">
							<label class="form-label">Status</label> <select
								class="form-select" name="offerStatus">
								<option value="PENDING"
									${offer.offerStatus == 'PENDING' ? 'selected' : ''}>PENDING</option>
								<option value="ACCEPTED"
									${offer.offerStatus == 'ACCEPTED' ? 'selected' : ''}>ACCEPTED</option>
								<option value="REJECTED"
									${offer.offerStatus == 'REJECTED' ? 'selected' : ''}>REJECTED</option>
							</select>
						...
						<div class="mb-4">
							<label class="form-label">Created Date</label> <input type="date"
								class="form-control" name="createdAt" value="${offer.createdAt}" />
						</div>
